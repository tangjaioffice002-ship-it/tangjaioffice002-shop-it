...
export default function Navbar() {
  ...
  return (
    <>
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 flex items-center justify-between h-14 sm:h-16">
          ...
          <a href="#home" className="flex items-center gap-3 group z-50 relative">
            <div className="relative w-10 h-10 sm:w-12 sm:h-12 bg-white rounded-xl shadow-md flex items-center justify-center border border-slate-100 group-hover:shadow-lg transition-all">
              <img
                src="/img/Logo.png"
                alt="TANGJAI"
            ...
            <div className="flex flex-col">
              <span
                className={`font-bold text-base sm:text-lg leading-tight transition-colors ${
                  scrolled || open
                    ? "text-slate-800"
                    : "text-slate-800 md:text-white md:mix-blend-difference"
                }`}
              >
                TANGJAI
              </span>
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="lg:hidden p-2 rounded-full bg-slate-100 text-slate-600 hover:bg-emerald-50 hover:text-emerald-600 transition-colors z-50"
            >
      ...
      {/* Drawer */}
      <div
        className={`fixed top-0 right-0 bottom-0 z-50 w-[85%] max-w-[330px] bg-white shadow-2xl 
          transition-transform duration-500 lg:hidden flex flex-col rounded-l-3xl
          ${
            open
              ? "translate-x-0"
              : "translate-x-full"
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {navItems.map((item, i) => (
            <a
              ...
              href={item.href}
              onClick={() => setOpen(false)}
              className="group flex items-center justify-between p-4 rounded-2xl bg-white border border-slate-100 shadow-md hover:shadow-xl hover:border-emerald-200 hover:bg-emerald-50/70 transition-all duration-300 active:scale-[0.98]"
            >
              <div className="flex items-center gap-4">
                <div className="w-11 h-11 rounded-xl bg-slate-100 text-slate-500 flex items-center justify-center group-hover:bg-emerald-500 group-hover:text-white transition-all duration-300 shadow-sm">
                  <item.icon size={20} />
                </div>
        ...
        <div className="p-6 bg-slate-50 border-t border-slate-100">
          <div className="mb-4 text-center">
            ...
            <p className="text-sm text-slate-600">ทีมงานพร้อมให้บริการเสมอ</p>
          </div>
